...
Gin + GORM + MySQL 8 的单二进制后端。本 README 面向在 `server/` 内开发的工程师,项目整体定位与部署见仓库根 [README](../README.md),API 明细见 [docs/openapi.yaml](../docs/openapi.yaml)。

## 目录结构

```text
server/
├── cmd/server/main.go   启动入口:配置 → 日志 → DB → 迁移 → HTTP → 优雅关闭
