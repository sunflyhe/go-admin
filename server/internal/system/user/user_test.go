...
func newEnv(t *testing.T) (*Service, *gin.Context) {
	t.Helper()
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	return NewService(testutil.NewTestDB(t)), c
}

func TestCreateAndDuplicate(t *testing.T) {
	svc, c := newEnv(t)
	item, err := svc.Create(c, &SaveReq{Username: "alice", Password: "12345678", Nickname: "Alice"})
	if err != nil {
	...
	if _, err := svc.Create(c, &SaveReq{Username: "alice", Password: "12345678"}); err == nil {
		t.Fatal("重复用户名应报冲突")
	}
}
...
func TestSuperAdminProtected(t *testing.T) {
	...
	if err := svc.SetStatus(c, 1, &SetStatusReq{Status: model.StatusDisabled}); err == nil {
		t.Fatal("内置超管账号不允许停用")
	}
}
...
func TestDeleteCleansRelations(t *testing.T) {
	svc, c := newEnv(t)
	testutil.SeedSuperAdmin(t, svc.DB)
	item, err := svc.Create(c, &SaveReq{Username: "bob", Password: "12345678"})
	if err != nil {
		t.Fatal(err)
	}
	if err := svc.AssignRoles(c, item.ID, &AssignRolesReq{RoleIDs: []int64{1}}); err != nil {
...
func TestResetPasswordInvalidatesToken(t *testing.T) {
	...
	if err := svc.ResetPassword(c, 1, &ResetPasswordReq{Password: "87654321"}); err != nil {
