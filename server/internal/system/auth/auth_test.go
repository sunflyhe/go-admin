...
func newAuthEnv(t *testing.T) (*gin.Engine, *auth.Manager, *model.SysUser) {
	t.Helper()
	gin.SetMode(gin.TestMode)
	db := testutil.NewTestDB(t)
	super := testutil.SeedSuperAdmin(t, db)
	jwt := auth.NewManager("test-secret-1234567890", 30*time.Minute, 24*time.Hour, "go-admin")
	svc := NewService(db, jwt, discardLogger())
	h := NewHandler(svc)
	authn := &mw.Authn{DB: db, JWT: jwt}
