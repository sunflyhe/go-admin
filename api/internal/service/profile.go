...
// Content 是否需要 Close 由构造方决定(multipart.File 需要);Service 只负责读取。
...
func (s *ProfileService) UploadAvatar(ctx context.Context, actor Actor, in AvatarInput) (string, error) {
	return s.UploadAvatarFor(ctx, actor, actor.ID, in)
}

// UploadAvatarFor 上传并绑定指定用户的头像,供用户管理(管理员改他人头像)复用
// 与个人中心完全相同的校验与回收链路。actor 是上传操作者(文件记录归属),
// targetID 是被改头像的用户,两者可以不同。
// 头像必须存成公开文件:<img> 带不了 Authorization,私有头像在页面上渲染不出来;
// 路径含随机 UUID 不可枚举,且落盘/元数据仍由 FileService 统一负责。
func (s *ProfileService) UploadAvatarFor(ctx context.Context, actor Actor, targetID int64, in AvatarInput) (string, error) {
	...
	if err := s.DB.WithContext(ctx).Model(&model.SysUser{ID: targetID}).
