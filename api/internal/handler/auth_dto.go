...
// LoginRequest POST /admin-api/auth/login 请求体。
type LoginRequest struct {
	Username string `json:"username" binding:"required,max=64"`
	Password string `json:"password" binding:"required,max=128"`
}

// RefreshRequest POST /admin-api/auth/refresh 请求体。
type RefreshRequest struct {
	RefreshToken string `json:"refreshToken" binding:"required"`
}

// ProfileUpdateRequest PUT /admin-api/auth/profile 请求体。
...
// ChangePasswordRequest POST /admin-api/auth/password 请求体。
...
// AvatarResponse POST /admin-api/auth/avatar 响应:头像最终访问地址。
