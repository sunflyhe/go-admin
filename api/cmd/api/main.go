// api 启动入口:加载配置 → 初始化日志 → 连接 MySQL → 执行迁移 → 启动 HTTP 服务,支持优雅关闭。
package main
...
import (
	...
	middleware "github.com/hesunfly/hesunfly-admin-go/api/internal/middleware"
	router "github.com/hesunfly/hesunfly-admin-go/api/internal/router"
	service "github.com/hesunfly/hesunfly-admin-go/api/internal/service"
	pkgauth "github.com/hesunfly/hesunfly-admin-go/api/pkg/auth"
	"github.com/hesunfly/hesunfly-admin-go/api/pkg/database"
	"github.com/hesunfly/hesunfly-admin-go/api/pkg/logger"
	"github.com/hesunfly/hesunfly-admin-go/api/pkg/migrate"
)
